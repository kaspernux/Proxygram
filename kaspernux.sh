...
colorized_echo green "Installing PROXYGRAM by Kapsernux . . ."
...
colorized_echo green "\n\tAll Proxygram robot files/folders have been successfully installed on your server!"
...
config_address="/var/www/html/ProxygramBot/install/zanbor.install"
...
# touch('/var/www/html/ProxygramBot/install/zanbor.install')
echo "{\"development\":\"@Proxygram\",\"install_location\":\"server\",\"main_domin\":\"${DOMAIN}\",\"token\":\"${TOKEN}\",\"dev\":\"${CHAT_ID}\",\"db_name\":\"${dbname}\",\"db_username\":\"${randdbdb}\",\"db_password\":\"${randdbpass}\"}" > /var/www/html/ProxygramBot/install/zanbor.install
...
curl --location "https://${DOMAIN}/ProxygramBot/sql/sql.php?db_password=${dbpass}&db_name=${dbname}&db_username=${dbuser}"

colorized_echo blue "\n\nSet Webhook Status:"
curl -F "url=https://${DOMAIN}/ProxygramBot/index.php" "https://api.telegram.org/bot${TOKEN}/setWebhook"

colorized_echo blue "\n\nSend Message Status:"
TEXT_MESSAGE="✅ The Proxygram Bot Has Been Successfully Installed !"
curl -s -X POST "https://api.telegram.org/bot${TOKEN}/sendMessage" -d chat_id="${CHAT_ID}" -d text="${TEXT_MESSAGE}"
echo -e "\n\n"

sleep 1
colorized_echo green "[+] The Proxygram Bot Has Been Successfully Installed"
colorized_echo green "[+] Telegram channel: @Proxygram || Telegram group: @ProxygramHUB"
echo -e "\n"

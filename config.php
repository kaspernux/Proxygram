...
function joinSend($from_id){
    ...
    if (count($buttons) > 0) {
        $buttons[] = [['text' => "I joined ✅", 'callback_data' => 'join']];
...
if ($from_id == $config['dev']) {
    if ($test_account_setting['status'] == 'active' and $user['test_account'] == 'no') {
        $start_key = json_encode(['keyboard' => [
            [['text' => '🔧 Управление']],
            [['text' => '🛍 Мои услуги'], ['text' => '🛒 Покупка услуг']],
            [['text' => '🎁 Тестовая услуга (бесплатно)']],
...
$confirm_service = json_encode(['keyboard' => [
    [['text' => '☑️ Создать услугу']], [['text' => '❌  Отмена']]
], 'resize_keyboard' => true]);

$select_diposet_payment = json_encode(['inline_keyboard' => [
    [['text' => '▫️Банковский перевод', 'callback_data' => 'kart']],
    [['text' => '▫️ZarinPal', 'callback_data' => 'zarinpal'], ['text' => '▫️IDPay', 'callback_data' => 'idpay']],
    [['text' => '▫️Валютный платеж', 'callback_data' => 'nowpayment']],
...
$panel = json_encode(['keyboard' => [
    [['text' => '📞 Уведомление об обновлении робота']],
    [['text' => '🔑 Система идентификации']],
    [['text' => '👥 Управление статистикой робота'], ['text' => '🌐 Управление сервером']],
    [['text' => '📤 Управление сообщениями'], ['text' => '👤 Управление пользователями']],
    [['text' => '⚙️ Настройки'], ['text' => '👮‍♂️Управление админами']],
    [['text' => '🔙 Назад']],
], 'resize_keyboard' => true]);

$manage_statistics = json_encode(['keyboard' => [
    [['text' => '👤 Статистика робота']],
    [['text' => '⬅️ Назад к управлению']]
], 'resize_keyboard' => true]);

$manage_server = json_encode(['keyboard' => [
    [['text' => '⏱ Управление тестовым аккаунтом']],
    [['text' => '⚙️ Управление тарифами'], ['text' => '🎟 Добавить тариф']],
    [['text' => '⚙️ Список серверов'], ['text' => '➕ Добавить сервер']],
    [['text' => '⬅️ Назад к управлению']]
], 'resize_keyboard' => true]);

$select_panel = json_encode(['inline_keyboard' => [
    [['text' => '▫Sanayi', 'callback_data' => 'sanayi']],
    [['text' => '▫️Hedifay', 'callback_data' => 'hedifay'], ['text' => '▫️Marzban', 'callback_data' => 'marzban']]
]]);

$add_plan_button = json_encode(['inline_keyboard' => [
    [['text' => '➕ Купить план услуги', 'callback_data' => 'add_buy_plan']],
    [['text' => '➕ План по времени', 'callback_data' => 'add_date_plan'], ['text' => '➕ План по объему', 'callback_data' => 'add_limit_plan']],
]]);

$manage_plans = json_encode(['inline_keyboard' => [
    [['text' => '🔧 Управление основным планом', 'callback_data' => 'manage_main_plan']],
    [['text' => '🔧 План по времени', 'callback_data' => 'manage_date_plan'], ['text' => '🔧 План по объему', 'callback_data' => 'manage_limit_plan']],
]]);
...
$manage_message = json_encode(['keyboard' => [
    [['text' => '🔎 Состояние отправки / пересылки всем']],
    [['text' => '📬 Переслать всем'], ['text' => '📬 Отправить всем']],
...
$manage_user = json_encode(['keyboard' => [
    ...
    [['text' => '➖ Уменьшить баланс'], ['text' => '➕ Увеличить баланс']],
...
$manage_setting = json_encode(['keyboard' => [
    ...
    [['text' => '◽ Настройка текстов робота'], ['text' => '◽ Настройки платежного шлюза']],
...
$manage_spam = json_encode(['inline_keyboard' => [
    ...
    [['text' => ($spam_setting['type'] == 'ban') ? '🚫 Заблокировать' : '⚠️ Предупреждение', 'callback_data' => 'change_type_spam'], ['text' => '▫️Метод обработки:', 'callback_data' => 'null']],
...
$manage_payment = json_encode(['keyboard' => [
    [['text' => '✏️ Состояние выключено/включено платежного шлюза робота']],
    [['text' => '▫️Настроить владельца номера карты'], ['text' => '▫️Настроить номер карты']],
    [['text' => '▫️ZarinPal'], ['text' => '▫️IDPay']],
    [['text' => '◽ NOWPayments']],
    [['text' => '⬅️ Вернуться к управлению']]
], 'resize_keyboard' => true]);
...
$manage_off_on_paymanet = json_encode(['inline_keyboard' => [
    ...
    [['text' => ($payment_setting['nowpayment_status'] == 'active') ? '🟢' : '🔴', 'callback_data' => 'change_status_nowpayment'], ['text' => ': NOWPayments ▫️', 'callback_data' => 'null']],
...
$manage_texts = json_encode(['keyboard' => [
    [['text' => '✏️ Текст тарифа услуг'], ['text' => '✏️ Текст старта']],
    [['text' => '✏️ Текст руководства по подключению']],
    [['text' => '⬅️ Вернуться к управлению']]
], 'resize_keyboard' => true]);
...
$back_services = json_encode(['inline_keyboard' => [
    [['text' => '🔙 Вернуться', 'callback_data' => 'back_services']]
]]);

$back_account_test = json_encode(['inline_keyboard' => [
    [['text' => '🔙 Вернуться', 'callback_data' => 'back_account_test']]
]]);

$back_spam = json_encode(['inline_keyboard' => [
    [['text' => '🔙 Вернуться', 'callback_data' => 'back_spam']]
]]);

$back_copen = json_encode(['inline_keyboard' => [
    [['text' => '🔙 Вернуться', 'callback_data' => 'back_copen']]
]]);

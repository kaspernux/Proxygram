...
class Sanayi{
    ...
    public function addClient($name, $id, $date, $limit){
        ...
        if($resp['success'] == true){
            return json_encode(['status' => true, 'msg' => 'успешно', 'results' => ['subscribe' => str_replace($parts['port'], $subPort, $this->base_url) . '/sub/' . $subid, 'id' => $randomKey, 'remark' => $name, 'limitIp' => 1, 'totalGB' => $total, 'expiryTime' => $date, 'subId' => $subid, 'subPort' => $subPort]], 448);
        } else {
            return json_encode(['status' => false, 'msg' => 'не успешно', 'results' => ['id' => $randomKey, 'remark' => $name, 'limitIp' => 1, 'totalGB' => $total, 'expiryTime' => $date, 'subId' => $subid]], 448);
        }
    ...
    public function getUserInfo($name, $id) {
        ...
        return json_encode(['status' => false, 'msg' => 'не найдено', 'status_code' => 404], 448);
    }
    ...
    public function addExpire($remark, $date, $id) {
        ...
        if($resp['success'] == true){
            return json_encode(['status' => true, 'msg' => 'успешно'], 448);
        } else {
            return json_encode(['status' => false, 'msg' => 'не успешно'], 448);
        }
    }
    ...
    public function addVolume($remark, $limit, $id) {
        ...
        if($resp['success'] == true){
            ...
            return json_encode(['status' => false, 'msg' => 'неудачно'], 448);
        }
    }
